...
import com.gdgistanbul.devfestistanbul.ui.AnimatedPathView;
import com.gdgistanbul.devfestistanbul.ui.TransitionAdapter;
...
//    }
    ...
    private void applyPalette(Palette palette) {
        ...
        int size = palette.getPallete().size();
        if (darkMuted==null)
            darkMuted=palette.getPallete().get(size - (size/8));
        if (muted==null)
            muted=palette.getPallete().get(size - (size/5));
        if (lightMuted==null)
            lightMuted=palette.getPallete().get(size - (size/3));
        if (vibrant==null)
            vibrant=palette.getPallete().get(size - (size/2));
        if (lightVibrant==null)
            lightVibrant=palette.getPallete().get(size - 1);
        if (darkVibrant==null)
            darkVibrant=palette.getPallete().get(size - (size/4));


        getWindow().setBackgroundDrawable(new ColorDrawable(darkMuted.getRgb()));

        TextView titleView = (TextView) findViewById(R.id.title);
        titleView.setTextColor(vibrant.getRgb());

        TextView descriptionView = (TextView) findViewById(R.id.description);
        descriptionView.setTextColor(lightVibrant.getRgb());

        colorRipple(R.id.info, darkMuted.getRgb(), darkVibrant.getRgb()
                );
        colorRipple(R.id.star, muted.getRgb(),
                vibrant.getRgb());

        View infoView = findViewById(R.id.information_container);
        infoView.setBackgroundColor(lightMuted.getRgb());

        AnimatedPathView star = (AnimatedPathView) findViewById(R.id.star_container);
        star.setFillColor(vibrant.getRgb());
        star.setStrokeColor(lightVibrant.getRgb());
    }
    ...
    private Bitmap setupPhoto(int resource) {
        ...
        //Bitmap bitmap= SessionData.sCache.get(0).getBitmap();
